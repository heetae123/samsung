...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('.post-item').forEach(item => {
    ...
    item.addEventListener('click', () => {
      const title = encodeURIComponent(item.querySelector('h4')?.textContent || item.querySelector('.post-title')?.textContent || "제품 정보");
      ...
      // 상세 페이지로 파라미터와 함께 이동
      window.location.href = `product-detail.html?title=${title}&price=${price}&discount=${discount}&video=${video}`;
    });
  });
});
